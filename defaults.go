...
func insertRecursively(into interface{}, from map[string]interface{}) {
	if v, ok := from["type"]; ok {
		...
		case "array":
			...
			// if array of objects, we step into it
		...
		case "object":
			...
			// an object with oneOf doesn't have properties, so return
			...
			for property, _nextSchema := range properties {
				...
				if v, ok := intoAsObject[property]; ok && v != nil {
					...
					// object.properties is always going to be map[string]interface{}, so
					// cast the generic []map type to that
					case reflect.Slice:
